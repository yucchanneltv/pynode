    ...
    async def interpret(self):
        for node in self.ast:
            if node['type'] == 'variable_assignment':
                ...
                await asyncio.gather(*(self.process_item(node['var'], item, node['body']) for item in collection))

    async def process_item(self, var_name, item, body):
        self.variables[var_name] = item
        print(f"For loop item: {item}, Body: {body}")
